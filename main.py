...
###################################################################
'''
     Facebook Information لشهب الزين
'''
###################################################################
#                             COLOR
...
print '{.HEADER}Facebook Information ®️   est une collecte précise des informations de compte facebook, toutes les informations sensibles peuvent être facilement collectées même si la cible convertit toute sa vie privée en informations confidentielles (uniquement pour moi), informations sensibles sur la résidence, la date de naissance, la profession, le numéro de téléphone et  l adresse électronique.{.ENDC}'.format(
    bcolors, bcolors)
...
#-*- encoding: utf-8 -*-
        ...
        print '' + W
    print ' ˜”*°•.˜”*°• %sTapper help pour entree%s •°*”˜.•°*”˜ '.center(
        88) % (G, W)
    print('ﻦﻳﺰﻟﺍ ﺐﻬﺸﻟ').center(77)
...
def show_program():
    ...
    ⎡ Author      ⎟   ﻦﻳﺰﻟﺍ ﺐﻬﺸﻟ                   ⎢
     >---------------------------------------------⟩
    ⎟ Name        ⎟   devchmoud fb Information     ⎢        
     >---------------------------------------------⟩
    ⎟ Version     ⎟   Full Version      😇         ⎢ 
     >---------------------------------------------⟫
    ⎟ Date        ⎟   13/08/2018                   ⎢
     >---------------------------------------------⟫
    ⎟ Mail        ⎟   lachhab-ezzine@outlook.fr    ⎢
    ---------------------♥︎♥︎♥︎♥︎♥︎♥︎♥︎-------------------
...
def info_ga():
   ...
   |dump_<id>_id  ⎮  fetching all id from your friends <spesific>  |
	 |      Ex: dump_username_id                  _____________|
   |-----------------------------------------------⁊
   |★ token         ⎮      Generate access token   |
   |-----------------------------------------------|
   |★ cat_token    ⎮      how your access token    |
   |-----------------------------------------------|
   |★ rm_token     ⎮      remove access token      |
   |-----------------------------------------------|
   |★ bot          ⎮      open bot menu            |︎ ︎ ︎
   |-----------------------------------------------|
   |★ clear        ⎮      clear terminal           |
   |-----------------------------------------------|
   |★ help         ⎮      show help                |
   |------------------------------------------------------------.
   |★ about        ⎮      Show information about this program ︎   |
   |------------------------------------------------------------ |
   |★ 𝐄𝑋ÍＴ        ⎮      Exit the program                      |
   '------------------------------------------------------------'
''' % (G, W)
...
def get(data):
    ...
    try:
        r = requests.get(
            'https://api.facebook.com/restserver.php', params=data)
        a = json.loads(r.text)
...
def remove(posts):
    ...
    try:
        ...
        for post in posts:
            ...
            r = requests.post(
                'https://graph.facebook.com/{id}?method=delete&access_token={token}'
                .format(id=post['id'], token=token))
            a = json.loads(r.text)
...
def unfollow(posts):
    ...
    try:
        ...
        for post in posts['data']:
            ...
            r = requests.post('https://graph.facebook.com/' + post['id'] +
                              '/subscribers?method=delete&access_token=' +
                              token)
            a = json.loads(r.text)
...
def poke(posts):
    ...
    try:
        ...
        for post in posts:
            ...
            r = requests.post(
                'https://graph.facebook.com/%s/pokes?access_token=%s' %
                (post['id'].split('_')[0], token))
            a = json.loads(r.text)
...
def menu_reaction_ask():
    try:
        ...
        cek = raw_input(R + 'devchmoud' + W + '/' + R + 'Bot' + W + '/' + R +
                        'Reaction' + W + ' >> ')
        ...
        if cek in ['1', '01']:
            ...
            print '\n' + '[*] Generate Access token facebook [*]'.center(
                44) + '\n'
...
def bot_ask():
    ...
    WT = raw_input(W + '[?] [' + R + 'W' + W + ']allpost or [' + R + 'T' + W +
                   ']arget (' + R + 'W' + W + '/' + R + 'T' + W + ') : ')
    if WT.upper() == 'T':
...
def bot():
    try:
        ...
        cek = raw_input(R + 'devchmoud' + W + '/' + R + 'Bot ' + W + '>> ')
        ...
        if cek in ['1', '01']:
            ...
            WT = raw_input(W + '[?] [' + R + 'W' + W + ']allpost or [' + R +
                           'T' + W + ']arget (' + R + 'W' + W + '/' + R + 'T' +
                           W + ') : ')
...
def dump_phone():
    ...
    try:
        ...
        for i in a['data']:
            x = requests.get("https://graph.facebook.com/" + i['id'] +
                             "?access_token=" + token)
            z = json.loads(x.text)

            try:
                out.write(z['mobile_phone'] + '\n')
                print W + '[' + G + z['name'] + W + ']' + R + ' >> ' + W + z[
                    'mobile_phone']
            except KeyError:
                pass
        out.close()
...
def dump_id_id():
    ...
    try:
        r = requests.get(
            'https://graph.facebook.com/{id}?fields=friends.limit(5000)&access_token={token}'
            .format(id=target_id, token=token))
        a = json.loads(r.text)

        out = open(
            'output/' + n[0].split(' ')[0] + '_' + target_id + '_id.txt', 'w')
        ...
        print '\r[*] all friends id successfuly retreived'
        print '[*] file saved : output/' + n[0].split(
            ' ')[0] + '_' + target_id + '_id.txt'
        main()
    except KeyboardInterrupt:
        print '\r[!] Stopped'
        main()
        ...
        print '[!] failed to fetch friend id'
        try:
            os.remove('output/' + n[0].split(' ')[0] + '_' + target_id +
                      '_id.txt')
        except OSError:
            pass
        main()
    ...
    for i in a['data']:
        ...
        if target in i['name'] or target in i['id']:
            ...
            try:
                ...
                pass
            try:
                print '[*] Updated time : ' + y['updated_time'][:10] + ' ' + y[
                    'updated_time'][11:19]
            except KeyError:
                pass
            try:
